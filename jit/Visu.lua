...
local Visu = {
    _VERSION = "1.1.0-jit",
    _DESCRIPTION = "Framework visuel tout-en-un avec particules, débogage et utilitaires (Optimisé LuaJIT)",
    _CONFIG = {}
}

-- Pre-allocate vector cache for better performance
local cache_vector = {}
for i = 1, 100 do
    cache_vector[i] = {x = 0, y = 0}
end
...
-- Use ffi for faster math operations when available
local abs = math.abs
...
function MathUtils.lerp(a, b, t)
    local clamped_t = t < 0 and 0 or (t > 1 and 1 or t)
...
function MathUtils.randomRange(min_val, max_val)
